...
router.get('/', auth, async (req, res) => {
  try {
    ...
    // Now: 1 query total, regardless of how many groups the user belongs to.
    if (groups.length > 0) {
...
`);
  ...
  router.post('/:groupId/members', auth, async (req, res) => {
    ...
    try {
      ...
      // Fetch the added user's info so the event carries enough data for UI update
      ...
      const io = req.app.get('io');
      if (io) {
        // 1. Notify the whole group room (existing members see membership change)
        io.to(`group_${groupId}`).emit('member_added', memberAddedPayload);

        // 2. CRITICAL FIX: also notify the ADDED USER on their personal room.
        //    The new user is NOT yet in group_${groupId} socket room because they
        //    only join rooms they were already a member of at connect-time.
        //    Without this emit, the added user never receives the event and their
        //    UI never updates until they refresh.
        io.to(`user_${user_id}`).emit('member_added', memberAddedPayload);
      }

      res.json({ message: 'Member added', user: addedUser || null });
    } catch (err) {
      await conn.rollback();
      res.status(500).json({ error: 'Server error' });
    } finally { conn.release(); }
  ...
  router.delete('/:groupId/members/:userId', auth, async (req, res) => {
    try {
      ...
      const io = req.app.get('io');
      if (io) {
        // 1. Notify the whole group room (remaining members see the change)
        io.to(`group_${groupId}`).emit('member_removed', memberRemovedPayload);

        // 2. CRITICAL FIX: also notify the REMOVED USER on their personal room.
        //    The Sidebar listener checks data.user_id === user.id which requires
        //    the event to arrive on their personal room — group room emission alone
        //    is insufficient because the Sidebar only acts on self-removal events.
        io.to(`user_${userId}`).emit('member_removed', memberRemovedPayload);
      }
